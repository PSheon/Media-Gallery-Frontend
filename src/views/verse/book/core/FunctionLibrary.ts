...
export function createCapsuleGeometry(radius = 1, height = 2, N = 32): THREE.Geometry {
  ...
  for (let i = 0; i <= N / 2; i++) {
    for (let j = 0; j < N; j++) {
      ...
      if (i === N / 4) {
        const face1 = new THREE.Face3(vec.x, vec.y, vec.z, [normals[vec.x], normals[vec.y], normals[vec.z]])

        const face2 = new THREE.Face3(vec.x, vec.z, vec.w, [normals[vec.x], normals[vec.z], normals[vec.w]])
