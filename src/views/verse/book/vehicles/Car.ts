...
export class Car extends Vehicle implements IControllable {
  ...
  public update(timeStep: number): void {
    ...
    if (this.shiftTimer > 0) {
      ...
      if (this.actions.reverse.isPressed) {
        ...
        const powerFactor =
          (gearsMaxSpeeds[this.gear] - this.speed) / (gearsMaxSpeeds[this.gear] - gearsMaxSpeeds[this.gear - 1])
  ...
  public physicsPreStep(body: CANNON.Body, car: Car): void {
    ...
    velocity.normalize()
    const driftCorrection = Utils.getSignedAngleBetweenVectors(Utils.threeVector(velocity), forward)
