  ...
  useEffect(() => {
    const checkProgress = async () => {
      const progress = (await readContract({
        ...
        args: ['1', (auth.user as UserDataType).address],
      ...
      <CardContent>
        ...
        <Box sx={{ mb: 1.25, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Typography variant='body2' sx={{ fontWeight: 600, color: 'text.primary' }}>
            {`${(mintedCount / 1860).toFixed(2)}%`}
          </Typography>
        </Box>
        <LinearProgress value={mintedCount / 1860} color='success' sx={{ mb: 5.75 }} variant='determinate' />
